...
from .multi_scale_deform_attn import NewMultiScaleDeformableAttention
...
class DeformableDetrTransformerDecoder(DetrTransformerDecoder):
    ...
    def forward(self,
        ...
        if self.return_intermediate:
            if return_attn:
                return torch.stack(intermediate),torch.stack(intermediate_attn), torch.stack(
                    intermediate_reference_points)
            else:
                return torch.stack(intermediate), torch.stack(
                    intermediate_reference_points)
        if return_attn:
            return output, reference_points,attn
        else:
...
class DeformableDetrTransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def _init_layers(self) -> None:
        ...
        self.cross_attn = NewMultiScaleDeformableAttention(**self.cross_attn_cfg)
        self.embed_dims = self.self_attn.embed_dims
        self.ffn = FFN(**self.ffn_cfg)
        norms_list = [
            build_norm_layer(self.norm_cfg, self.embed_dims)[1]

...
@TRANSFORMS.register_module()
class LoadActiveAnnotations(MMCV_LoadAnnotations):
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(LoadActiveAnnotations, self).__init__(**kwargs)
    ...
    def _load_labels(self, results: dict) -> None:
        ...
        """
        gt_bboxes_labels = []
        gt_bboxes_labels_obj = []
        for instance in results.get('instances', []):
            gt_bboxes_labels.append(instance['bbox_label'])
            gt_bboxes_labels_obj.append(instance['bbox_label_obj'])
        # TODO: Inconsistent with mmcv, consider how to deal with it later.
        results['gt_bboxes_labels'] = np.array(
            gt_bboxes_labels, dtype=np.int64)
        results['gt_bboxes_labels_obj'] = np.array(
            gt_bboxes_labels_obj, dtype=np.int64)
